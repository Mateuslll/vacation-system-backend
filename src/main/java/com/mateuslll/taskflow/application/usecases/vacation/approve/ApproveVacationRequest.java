...
public class ApproveVacationRequest implements ApproveVacationRequestUseCase {
    ...
    @Override
    @Transactional
    public VacationRequestResponseDTO execute(UUID vacationRequestId, UUID managerId) {
        ...
        vacationRequest.approve(managerId);
    ...
    private void validateManagerHierarchy(User manager, User collaborator) {
        ...
        if (!collaborator.isManagedBy(manager.getId())) {
            throw new ForbiddenAccessException(
                    "Manager " + manager.getFullName() + " não pode aprovar férias de " +
